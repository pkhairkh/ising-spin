...
CONTRASTIVE LEARNING (v71 — PCD RESTORED):
  v28 removed PCD based on the argument that "at right sparsity, Hebbian is
  the RG fixed point" (Agliari 2025, Eugenio 2025). This is TRUE for pattern
  completion (recovering a stored pattern from partial input) but FALSE for
  discriminative next-word prediction (choosing between thousands of candidates).

  Without the negative phase, Hebbian-only J creates:
    - Many shallow spurious energy basins (partial pattern overlaps)
    - Low SNR: all candidates get ~same energy (~-1000 ± small noise)
    - Good PPL (average probability is OK) but word salad generation
      (can't discriminate the right word from wrong words)

  PCD carves the energy landscape: deepens real basins, shallows spurious ones.
  The model's "fantasy" states (attractor dynamics from context) reveal spurious
  correlations — and PCD weakens exactly those couplings.

  v71 restores integer-only PCD at L0 after batch Hebbian:
    1. Batch Hebbian stores all positive correlations (as before)
    2. PCD runs attractor dynamics to generate fantasy states
    3. Anti-Hebbian update weakens fantasy correlations
    4. Net: ΔJ = η*(data_corr - model_corr) — classic contrastive divergence
...
class DAMLayer:
    ...
    Learning: Hebbian (positive phase) + PCD contrastive (negative phase).
    v71: PCD restored — Hebbian alone creates spurious basins that cause
    word salad. Contrastive learning carves the energy landscape for
    discriminative next-word prediction.
    """
    ...
    def __init__(
        ...
        pcd_alpha: int = 25,  # v75: PCD negative phase strength (Q8: 25=0.25, 50=0.5, 100=1.0)
        seed: int = 42,
    ):
        """
        Args:
            ...
            pcd_alpha: PCD contrastive learning strength in Q8 fixed-point.
                       25 = 0.25x (gentle, v73/v74 default), 50 = 0.5x (moderate),
                       100 = 1.0x (full). Controls how strongly the negative phase
                       weakens spurious couplings.
            seed: Random seed.
        """
        self.D = D
        self.k = k
        ...
        # Build log2(F) lookup table for energy computation (v33)
        # log2(F(x)) = log2(T + r) + n  where n = x//T, r = x%T
        # Precompute log2(T+r) for all valid r in [0, 2T-1] in 256x fixed-point
    ...
    def store_hebbian(
        ...
        This IS the positive phase of learning. The negative phase (PCD)
        is applied separately via pcd_update() to suppress spurious basins.
    ...
    def pcd_update(
        ...
        """
        v72: Persistent Contrastive Divergence — PROPER UNCONDITIONAL PCD.

        v71 BUG: The old PCD ran attractor dynamics FROM the context field
        (conditional), producing fantasy states that partially overlapped with
        the data targets. Then it SUBTRACTED these overlaps from J. This is
        COUNTERPRODUCTIVE — it weakens the CORRECT couplings!

        The whole point of PCD is to subtract the MODEL'S EQUILIBRIUM
        distribution, not the data-conditioned predictions. The model's
        equilibrium (unconditional) distribution contains spurious basins —
        attractor states the model falls into when running free, without any
        context. These are the states that produce word salad during generation.

        v72 FIX: Use PERSISTENT fantasy particles that evolve under the model's
        UNCONDITIONAL dynamics (just J and h, no context field). These particles
        represent the model's equilibrium distribution. We subtract the
        auto-correlation of these fantasies from J — this weakens exactly the
        spurious basins that cause word salad.

        Algorithm (proper PCD):
          1. POSITIVE: Already done by Hebbian — J += target^T @ context
          2. NEGATIVE: Run UNCONDITIONAL MCMC on persistent fantasy particles
          3. Subtract: J -= (N/M) * pcd_alpha * fantasy^T @ fantasy (auto-corr)
          4. Net: data basins deepened, model equilibrium basins shallowed

        The fantasy particles are PERSISTENT — they carry over between batches.
        This is the key insight of PCD vs CD: the particles don't restart from
        random each time; they continue from where they left off, which means
        they explore the model's equilibrium distribution more thoroughly.

        Args:
            context_sdrs: Context SDRs (N, D) uint8 — used only for positive phase stats.
            target_sdrs: Target SDRs (N, D) uint8 — used only for positive phase stats.
            eta: Anti-Hebbian learning rate (integer). Default 1.
            n_dream_steps: Number of MCMC sweeps per PCD step. Default 5.

        Returns:
            Diagnostics dict with PCD statistics.
        """
        N = context_sdrs.shape[0]
        D = self.D
        k = self.k
        if N == 0:
            return {'pcd_pairs': 0, 'fantasy_overlap': 0}

        # Initialize persistent fantasy particles on first call
        n_fantasy = min(200, D)
        if not hasattr(self, '_pcd_fantasy') or self._pcd_fantasy is None or self._pcd_fantasy.shape[0] != n_fantasy:
            self._pcd_fantasy = np.zeros((n_fantasy, D), dtype=np.uint8)
            rng = np.random.RandomState(self.seed + 999)
            for i in range(n_fantasy):
                active = rng.choice(D, k, replace=False)
                self._pcd_fantasy[i, active] = 1
        M = n_fantasy

        # === NEGATIVE PHASE: Evolve fantasy particles under UNCONDITIONAL dynamics ===
        # No context field — just J@state + h + small noise
        J_float = self.J.astype(np.float32)    # (D, D) — symmetric
        h_float = self.h.astype(np.float32)    # (D,)
        noise_scale = 10  # Small noise for diversity (prevents collapse)

        for _ in range(n_dream_steps):
            # Vectorized field: fields[i] = J @ fantasy[i] + h
            fields = self._pcd_fantasy.astype(np.float32) @ J_float
            fields += h_float[np.newaxis, :]

            # Small noise for diversity
            if noise_scale > 0:
                rng = getattr(self, '_pcd_rng', np.random.RandomState(self.seed + 777))
                noise = rng.randint(-noise_scale, noise_scale + 1,
                                   size=fields.shape).astype(np.float32)
                fields += noise
                self._pcd_rng = rng

            # kWTA: keep top k per particle
            fields_int = np.round(fields).astype(np.int32)
            top_k_all = np.argpartition(fields_int, -k, axis=1)[:, -k:]

            # Build new fantasy states
            self._pcd_fantasy = np.zeros((M, D), dtype=np.uint8)
            rows = np.arange(M).reshape(-1, 1)
            self._pcd_fantasy[rows, top_k_all] = 1

        # === COMPUTE NEGATIVE STATISTICS: fantasy auto-correlation ===
        # This represents the model's EQUILIBRIUM distribution — the spurious basins
        fan_f = self._pcd_fantasy.astype(np.float32)  # (M, D)
        J_neg_f = fan_f.T @ fan_f                      # (D, D) auto-correlation

        # Scale: balance N positive samples against M negative particles
        # pcd_alpha: controls negative phase strength (Q8: 100 = 1.0)
        # v75: pcd_alpha is now a constructor parameter (was hardcoded at 25).
        # The Q8 convention: 100 = 1.0 (full), 25 = 0.25 (gentle), 50 = 0.5 (moderate).
        pcd_alpha = self._pcd_alpha
        scale_factor = float(N) / float(M) * float(pcd_alpha) / 100.0
        J_neg_f *= scale_factor
        J_neg = J_neg_f.astype(np.int32)
        del fan_f, J_neg_f

        np.fill_diagonal(J_neg, 0)
        J_neg = (J_neg + J_neg.T) // 2

        # === APPLY: J -= J_neg (weaken model equilibrium correlations) ===
        J_new = self.J.astype(np.int32) - J_neg * eta
        np.clip(J_new, -self.j_clip, self.j_clip, out=J_new)
        self.J = J_new.astype(np.int16)

        # Bias: weaken fantasy-active biases
        h_anti = np.sum(self._pcd_fantasy.astype(np.int32), axis=0) * eta
        h_anti = (h_anti * pcd_alpha) // 100  # Same scaling
        h_new = self.h.astype(np.int32) - h_anti
        np.clip(h_new, -self.j_clip, self.j_clip, out=h_new)
        self.h = h_new.astype(np.int16)

        # Diagnostics: fantasy particle diversity and overlap with data
        if M >= 10:
            rng = getattr(self, '_pcd_rng', np.random.RandomState(42))
            sample_idx = rng.choice(M, min(10, M), replace=False)
            sample = self._pcd_fantasy[sample_idx].astype(np.int32)
            overlaps = []
            for i in range(len(sample)):
                for j in range(i + 1, len(sample)):
                    overlaps.append(int(np.sum(sample[i] & sample[j])))
            avg_fantasy_overlap = float(np.mean(overlaps)) if overlaps else 0.0
        else:
            avg_fantasy_overlap = 0.0

        # Also measure overlap between fantasy and targets
        data_overlaps = []
        for i in range(min(N, 50)):
            tgt_active = set(np.where(target_sdrs[i] > 0)[0])
            # Pick a random fantasy
            fi = i % M
            fan_active = set(np.where(self._pcd_fantasy[fi] > 0)[0])
            if len(tgt_active) > 0:
                data_overlaps.append(len(tgt_active & fan_active) / len(tgt_active))
        avg_data_overlap = float(np.mean(data_overlaps)) if data_overlaps else 0.0

        self._stats['pcd_updates'] = self._stats.get('pcd_updates', 0) + N

        return {
            'pcd_pairs': N,
            'fantasy_overlap': avg_fantasy_overlap,
            'data_overlap': avg_data_overlap,
            'J_neg_max': int(np.max(np.abs(J_neg))),
            'J_neg_nnz': int(np.sum(J_neg != 0)),
        }
    ...
    def apply_update(
        ...
        """
        v76: Apply an external J and h update (from NCE trainer).

        This method is called by the NCETrainer to apply the NCE Hebbian
        update: J += J_update, h += h_update, with subsequent clipping,
        diagonal zeroing, and symmetrization.

        This is separate from store_batch_hebbian() because NCE computes
        its own update (positive - negative phases) rather than using the
        simple Hebbian outer product. The NCE trainer needs fine-grained
        control over the update to subtract the negative phase.

        Steps:
          1. Apply J_update to self.J with clipping to [-j_clip, +j_clip]
          2. Apply h_update to self.h with clipping
          3. Zero diagonal of J (no self-coupling)
          4. Symmetrize J (J = (J + J.T) // 2)
          5. Apply UV regularization if enabled

        Args:
            J_update: Coupling matrix update (D, D) int32.
            h_update: Bias update (D,) int32.
        """
        # Apply J update with clipping
        J_new = self.J.astype(np.int32) + J_update
        np.clip(J_new, -self.j_clip, self.j_clip, out=J_new)
        self.J = J_new.astype(np.int16)

        # Apply h update with clipping
        h_new = self.h.astype(np.int32) + h_update
        np.clip(h_new, -self.j_clip, self.j_clip, out=h_new)
        self.h = h_new.astype(np.int16)
        ...
        # Symmetrize J
        self.J = ((self.J.astype(np.int32) + self.J.T.astype(np.int32)) // 2).astype(np.int16)

        # UV regularization if enabled
