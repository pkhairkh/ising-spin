#!/usr/bin/env python3
"""
PoC Runner for v6 Ising Spin Language Model — Principled MCMC Rewrite.

Replaces v5's ugly workarounds (history-driven target, momentum tracker)
with three PRINCIPLED mathematical solutions:

  1. Swendsen-Wang Cluster Algorithm (FK random-cluster representation)
  2. Proper Parallel Tempering (geometric ladder, precomputed swap tables)
  3. Entropy-Regularized Free Energy (F = E - T_ent * S)
"""
...
from ising_spin.enhanced_v6_model import EnhancedV6Model
from ising_spin.type_system import IDX2POS, POS2IDX, N_POS


def run_v6_poc():
    print("=" * 70)
    print("ISING SPIN LANGUAGE MODEL v6 — PRINCIPLED MCMC PoC")
    print("=" * 70)
    print()
    print("v6 Replaces v5's workarounds with PRINCIPLED solutions:")
    print("  P3a: Swendsen-Wang cluster moves (Fortuin-Kasteleyn-SW)")
    print("  P3b: Proper parallel tempering (geometric ladder, 8 replicas)")
    print("  P3c: Entropy-regularized free energy (F = E - T_ent * S)")
    print("  REMOVED: History-driven target (violated detailed balance)")
    print("  REMOVED: Momentum tracker (broke detailed balance)")
    print()

    model = EnhancedV6Model(
        # Vocabulary
        vocab_min_freq=5,
        vocab_max_size=3000,
        # Sequence
    ...
    model.save("data/v6_model")

    # Generate samples
    print("\n" + "=" * 70)
    print("GENERATION SAMPLES (v6: Principled MCMC)")
    print("=" * 70)
    ...
    # v5 vs v6 comparison
    print("\n" + "=" * 70)
    print("v5 → v6 CHANGE SUMMARY")
    print("=" * 70)
    print("  REMOVED: HistoryBuffer (violated detailed balance)")
    print("  REMOVED: MomentumTracker (broke detailed balance, meaningless in word space)")
    print("  ADDED:   Swendsen-Wang cluster moves (FK representation, preserves DB)")
    print("  ADDED:   Proper PT with geometric ladder + precomputed swap tables")
    print("  ADDED:   Entropy-regularized free energy F = E - T_ent * S (state function)")
    print("  FIXED:   PT swap uses math.exp() at runtime → now precomputed integer table")
    print("  FIXED:   Arithmetic beta spacing → geometric (Katzgraber et al., 2006)")
    print("  FIXED:   4 replicas → 8 replicas with adaptive tuning")
    ...
    with open("data/v6_model/poc_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to data/v6_model/poc_results.json")
