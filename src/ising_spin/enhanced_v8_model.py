"""
Enhanced Typed Ising-Potts Language Model v8 — Marginal MaxEnt Anti-Repetition.
...
Builds on v7 (Demon algorithm, Wang-Landau DOS, Lifted MCMC) and adds:
  ...
  P4d: Marginal Maximum Entropy on Per-Position Consecutive Counts
     ...
     - Derived from Jaynes' MaxEnt principle applied to the marginal
       distribution P(X_i):
       * If position i has had the same word for K consecutive sweeps,
         the empirical P(w_old | i) ≈ 1, entropy H(X_i) ≈ 0
       * The surprisal of STAYING is low; the surprisal of LEAVING is high
       * We HELP escape by subtracting T_ent * S_i from delta_E
     - The bonus for LEAVING a stuck position is:
         bonus = T_ent * log2(consecutive_count[i] + 1) * SCALE / 1000
     - This makes delta_F = delta_E - bonus, easier to accept change

  Detailed Balance Preservation:
     - Consecutive counts are FROZEN in a snapshot at the START of each sweep
     - All acceptance decisions within a sweep use the snapshot
     - After the sweep, counts are updated based on the new configuration
     - Same approach as the multicanonical algorithm (Berg & Neuhaus 1991)
...
# =============================================================================
# P4d: Marginal Entropy Tracker — Per-Position MaxEnt Anti-Repetition
# =============================================================================

class MarginalEntropyTracker:
    """
    Per-position marginal entropy tracker for MaxEnt anti-repetition.

    Tracks consecutive visit counts for each position and computes
    the marginal surprisal S_i = log2(consecutive_count[i] + 1) * SCALE.

    Derived from Jaynes' MaxEnt principle applied to the marginal P(X_i):
    - If position i has had the same word for K consecutive sweeps, the
      empirical P(w_old | i) ≈ 1, so H(X_i) ≈ 0 (completely stuck).
    - The surprisal of STAYING = -log2(P(w_old)) ≈ 0 (no surprise).
    - The surprisal of LEAVING = -log2(1 - P(w_old)) → ∞ (very surprising).
    - We want to HELP escape: the bonus for leaving is T_ent * S_i, where
      S_i increases with consecutive count.

    Detailed balance is preserved by using a SNAPSHOT of the consecutive
    counts at the START of each sweep. The counts are FROZEN during the
    sweep and only updated BETWEEN sweeps. This is the same approach as
    the multicanonical algorithm (Berg & Neuhaus 1991) — update weights
    between runs, not during. The diminishing adaptation condition is
    satisfied because the relative impact of each new observation
    decreases as 1/N.
    """

    def __init__(self, seq_length: int, scale: int = 10000):
        """
        Args:
            seq_length: Number of positions in the sequence.
            scale: Fixed-point scale for log2 computation.
                   S_i = log2(count+1) * scale, then divided by 1000
                   when computing the bonus.
        """
        self.L = seq_length
        self.SCALE = scale
        self.consecutive_count = np.zeros(seq_length, dtype=np.int64)
        self.last_word = np.full(seq_length, -1, dtype=np.int64)  # -1 = uninitialized
        self.snapshot_count: Optional[np.ndarray] = None  # Frozen during sweep
        self._total_sweeps = 0
        self._total_updates = 0
        self._total_flips = 0  # positions that changed word

    def snapshot(self):
        """Take snapshot of consecutive counts for current sweep.

        This MUST be called before each sweep. The snapshot is FROZEN
        during the sweep, preserving detailed balance.
        """
        self.snapshot_count = self.consecutive_count.copy()

    def get_surprisal(self, pos: int) -> int:
        """Get marginal surprisal at position pos (from snapshot).

        S_i = log2(consecutive_count[i] + 1) * SCALE

        Uses bit_length() - 1 for floor(log2) and linear interpolation
        for the fractional part. All integer arithmetic.

        If no snapshot exists (e.g. before first sweep), uses the
        live consecutive_count (which is safe because the state hasn't
        changed yet).
        """
        if self.snapshot_count is not None:
            count = int(self.snapshot_count[pos])
        else:
            count = int(self.consecutive_count[pos])

        # S = log2(count + 1) * SCALE
        n = count + 1
        s_int = n.bit_length() - 1  # floor(log2(n))
        if n > (1 << s_int):
            frac = ((n - (1 << s_int)) * self.SCALE) // (1 << s_int)
        else:
            frac = 0
        return s_int * self.SCALE + frac

    def get_max_surprisal(self) -> int:
        """Get maximum surprisal across all positions (from snapshot)."""
        counts = self.snapshot_count if self.snapshot_count is not None else self.consecutive_count
    ...
    def update(self, pos: int, word: int):
        """Update consecutive count for a single position after a sweep.

        If the word is the same as last time, increment the count.
        If the word changed, reset the count to 0 and record the new word.
        """
        self._total_updates += 1
        if word == self.last_word[pos]:
            self.consecutive_count[pos] += 1
        else:
            self.consecutive_count[pos] = 0
            self.last_word[pos] = word
            self._total_flips += 1

    def update_all(self, state_words):
        """Update all positions after a sweep.

        This MUST be called after each sweep, BEFORE taking the next snapshot.
        """
        self._total_sweeps += 1
        for i in range(self.L):
            self.update(i, state_words[i])
    ...
    def get_stuck_positions(self, threshold: int = 5) -> List[int]:
        """Get positions that have been stuck for >= threshold sweeps."""
        counts = self.consecutive_count if self.snapshot_count is None else self.snapshot_count
        return [int(i) for i in range(self.L) if counts[i] >= threshold]

    def get_stats(self) -> Dict:
        """Get tracker statistics (FP allowed — diagnostics only)."""
        counts = self.consecutive_count
        stuck_5 = int((counts >= 5).sum())
        stuck_10 = int((counts >= 10).sum())
        stuck_20 = int((counts >= 20).sum())
        avg_count = float(counts.mean()) if len(counts) > 0 else 0.0
        max_count = int(counts.max()) if len(counts) > 0 else 0
        return {
            "total_sweeps": self._total_sweeps,
            "total_updates": self._total_updates,
            "total_flips": self._total_flips,
            "flip_rate": self._total_flips / max(1, self._total_updates),
            "avg_consecutive_count": avg_count,
            "max_consecutive_count": max_count,
            "stuck_5plus": stuck_5,
            "stuck_10plus": stuck_10,
            "stuck_20plus": stuck_20,
            "SCALE": self.SCALE,
        }
...
# =============================================================================
# V8 Enhanced Sampler — v7 + Marginal MaxEnt Anti-Repetition
# =============================================================================

class EnhancedV8Sampler:
    """
    V8 Enhanced sampler with Marginal MaxEnt anti-repetition on top of v7.
    ...
    New in v8:
      P4d: Marginal Maximum Entropy on Per-Position Consecutive Counts
           ...
           - The bonus for LEAVING a stuck position:
             bonus = T_ent * S_i / 1000
           - This is subtracted from delta_E, making acceptance easier
           - Derived from Jaynes' MaxEnt: P(X_i) peaked at one word → low entropy → need help escaping
           - Detailed balance preserved via snapshot mechanism (Berg & Neuhaus 1991)
    ...
    def __init__(
        ...
        # P4d: Marginal MaxEnt
        marginal_entropy_scale: int = 10000,
        ...
        # P4d: Marginal MaxEnt anti-repetition
        self.use_marginal_entropy = use_marginal_entropy
        self.marginal_entropy_scale = marginal_entropy_scale
        self.marginal_entropy: Optional[MarginalEntropyTracker] = None  # initialized per generation
    ...
    # =========================================================================
    # Standard MCMC Infrastructure (from v5/v6/v7, preserved)
    # =========================================================================
    ...
    # =========================================================================
    # Type Sampling (same as v5/v6/v7 — preserved)
    # =========================================================================
    ...
    def _sample_type(self, pos, current_type, state_types):
        ...
        type_weights = np.maximum(max_e - type_energies + 1, 0)

        # Zero out disallowed types
        if self.allowed_transitions is not None:
            for t in range(self.n_types):
                if pos > 0:
                    if (state_types[pos - 1], t) not in self.allowed_transitions:
        ...
        total_w = int(type_weights.sum())
        if total_w <= 0:
            return current_type

        cumsum = np.cumsum(type_weights)
        return self._sample_from_cumsum(cumsum)

    # =========================================================================
    # Energy Computation (same as v7, preserved)
    # =========================================================================
    ...
    def _compute_entropy_regularized_energy(
        ...
        This is the v7-style entropy regularization at the TOTAL energy level.
        In v8, the MARGINAL entropy (P4d) is applied separately via the
        acceptance criterion, not here. This function is kept as a fallback
        when marginal entropy is disabled.
        """
    ...
    # =========================================================================
    # P4d: Marginal Entropy Bonus Computation
    # =========================================================================

    def _compute_marginal_entropy_bonus(self, pos: int) -> int:
        """
        Compute the marginal entropy bonus for LEAVING position pos.

        bonus = T_ent * S_i / 1000
        where S_i = log2(consecutive_count[pos] + 1) * SCALE

        This is a STATE FUNCTION of the current configuration's snapshot.
        It does NOT depend on the proposed word — only on how long the
        current word has been stuck at position pos.

        The bonus is SUBTRACTED from delta_E in the acceptance criterion,
        making it easier to accept a change when a word has been stuck.
        """
        if self.marginal_entropy is None:
            return 0
        S_i = self.marginal_entropy.get_surprisal(pos)
        return (self.T_ent * S_i) // 1000

    # =========================================================================
    # Locally-Balanced Proposal (Zanella 2017) — with Demon + Marginal Entropy
    # =========================================================================
    ...
    def _locally_balanced_proposal(
        ...
        """P0+P3c+P4a+P4d: Locally-balanced proposal with Demon + Marginal entropy acceptance."""
        candidates = []
        seen = {current_word}
        ...
        if use_entropy and self.T_ent > 0 and not self.use_marginal_entropy:
            # v7 fallback: use total-energy Boltzmann entropy
        ...
        # P4d: Apply marginal entropy bonus
        # The bonus is SUBTRACTED from delta_e, making it easier to accept
        # a change when the current word has been stuck for many sweeps.
        # This is the KEY v8 improvement over v7.
        if self.use_marginal_entropy and self.marginal_entropy is not None:
            bonus = self._compute_marginal_entropy_bonus(pos)
            delta_e_exact -= bonus
    ...
    # =========================================================================
    # Swendsen-Wang Cluster Sweep (with P4d marginal entropy bonus)
    # =========================================================================
    ...
    def _sw_sweep(
        ...
        """One Swendsen-Wang cluster sweep with marginal entropy bonus."""
        L = len(state_words)
        ...
        for root, positions in clusters.items():
            if len(positions) == 1:
                ...
                # P4d: Apply marginal entropy bonus for each position in cluster
                if self.use_marginal_entropy and self.marginal_entropy is not None:
                    cluster_bonus = 0
                    for pos in positions:
                        cluster_bonus += self._compute_marginal_entropy_bonus(pos)
                    delta_e -= cluster_bonus
                elif use_entropy and self.T_ent > 0:
                    # v7 fallback: use local entropy for cluster
                    entropy_delta = 0
                    for pos in positions:
    ...
    # =========================================================================
    # Wolff Single-Cluster Sweep (with P4d marginal entropy bonus)
    # =========================================================================
    ...
    def _wolff_sweep(
        ...
        """Wolff single-cluster variant of Swendsen-Wang with marginal entropy bonus."""
        L = len(state_words)
        if L <= prompt_len:
            return H_sum
    ...
    # =========================================================================
    # Running Energy Histogram for Boltzmann Entropy (v7 fallback)
    # =========================================================================
    ...
    # =========================================================================
    # Generation (v8: WL warm-up + Demon + Lifted + Marginal MaxEnt)
    # =========================================================================

    def generate(self, length=20, prompt=None, vocab=None, verbose=False):
        """
        Generate text with v8 enhancements.

        Phase 0: Energy calibration + Demon setup + Marginal entropy init
        Phase 1: Type annealing (exact enumeration over 13 types)
        Phase 2: Types + Words (Demon + Marginal entropy + Lifted + SW + PT)
        Phase 3: Words only (Demon + Marginal entropy + Lifted + SW, types frozen)

        The KEY v8 difference: P4d Marginal MaxEnt applies anti-repetition
        at the PER-POSITION level, not at the total energy level.
        """
        # Encode prompt
        ...
        # P4d: Initialize marginal entropy tracker
        if self.use_marginal_entropy:
            self.marginal_entropy = MarginalEntropyTracker(
                seq_length=length, scale=self.marginal_entropy_scale
            )
            # Initialize last_word from the current state
            for i in range(length):
                self.marginal_entropy.last_word[i] = state_words[i]
            # Take initial snapshot (all counts are 0, so no bonus yet)
            self.marginal_entropy.snapshot()
        else:
        ...
        # Use a RUNNING ENERGY HISTOGRAM as v7 fallback when marginal
        # entropy is disabled. When marginal entropy is enabled, the
        # histogram is less important but still kept for diagnostics.
        ...
        # Initialize running energy histogram for Boltzmann entropy (v7 fallback)
        ...
        self._energy_histogram_scale = 10000
        ...
        if verbose:
            ...
            print(f"    P4d Marginal entropy: {'ON' if self.use_marginal_entropy else 'OFF'}")
            if self.use_marginal_entropy:
                print(f"    Marginal entropy SCALE: {self.marginal_entropy_scale}")
                print(f"    T_ent: {self.T_ent}")
        ...
        # P4d: Update marginal entropy with Phase 1 state
        # (Phase 1 only changes types, not words, so consecutive counts
        # for words may increase. We update_all to reflect the state.)
        if self.marginal_entropy is not None:
            self.marginal_entropy.update_all(state_words)
            self.marginal_entropy.snapshot()

        if verbose:
            self._print_state(state_words, state_types, vocab, "After Phase 1")

        # ============ PHASE 2: Types + Words with PT + Demon + Marginal entropy + Lifted ============
        for sweep in range(self.sweeps_p2):
            # P4d: Take snapshot of consecutive counts for this sweep
            if self.marginal_entropy is not None:
                self.marginal_entropy.snapshot()
                ...
                if self.sw_wolff_variant:
                    ...
                    for _ in range(n_wolff):
                        H_sum = self._wolff_sweep(
                            ...
                            use_entropy=(not self.use_marginal_entropy)
                        )
                else:
            ...
            # P4d: Update marginal entropy with new state AFTER sweep
            if self.marginal_entropy is not None:
                self.marginal_entropy.update_all(state_words)

            # Record energy for Boltzmann entropy (diagnostics / fallback)
            ...
            if verbose and (sweep + 1) % max(1, self.sweeps_p2 // 4) == 0:
                self._print_state(state_words, state_types, vocab,
                                  f"P2 sweep {sweep+1}")
                if self.marginal_entropy is not None:
                    me_stats = self.marginal_entropy.get_stats()
                    print(f"      Marginal: avg_count={me_stats['avg_consecutive_count']:.1f}, "
                          f"max_count={me_stats['max_consecutive_count']}, "
                          f"stuck5+={me_stats['stuck_5plus']}, "
                          f"flip_rate={me_stats['flip_rate']:.3f}")
...
class EnhancedV8Model:
    """
    V8 Enhanced Typed Ising-Potts Language Model.

    Adds Marginal MaxEnt anti-repetition on top of v7:
      P4d: Marginal Maximum Entropy on Per-Position Consecutive Counts
           - Replaces coarse total-energy Boltzmann entropy with PER-POSITION
             marginal surprisal S_i = log2(consecutive_count[i] + 1) * SCALE
           - The bonus for LEAVING a stuck position:
             bonus = T_ent * log2(consecutive_count[i] + 1) * SCALE / 1000
           - This is SUBTRACTED from delta_E, making acceptance easier
           - Derived from Jaynes' MaxEnt: stuck position has low marginal entropy → need help
    ...
    def train(self, n_samples=50000, verbose=True):
        """Train the v8 model."""
        print("=" * 70)
        print("ENHANCED TYPED ISING-POTTS MODEL v8 — MARGINAL MAXENT")
        ...
        print("(P4d: Marginal MaxEnt — per-position anti-repetition)")
        ...
        print(f"(Marginal MaxEnt: {'ON' if self.use_marginal_entropy else 'OFF'}, "
              f"SCALE={self.marginal_entropy_scale}, T_ent={self.entropy_T_ent})")
        print("=" * 70)
        ...
        # Build sampler
        print("\nBuilding v8 sampler (P0+P1+P3a+P3b+P3c+P4a+P4b+P4c+P4d)...")
        t0 = time.time()
        self.sampler = EnhancedV8Sampler(
            ...
            marginal_entropy_scale=self.marginal_entropy_scale,
            use_marginal_entropy=self.use_marginal_entropy,
        )
        print(f"Sampler ready ({time.time()-t0:.1f}s)")

        # Summary
        print("\n" + "=" * 70)
        print("TRAINING COMPLETE — v8 ENHANCED MODEL (P0+P1+P3a+P3b+P3c+P4a+P4b+P4c+P4d)")
        print("=" * 70)
        self._print_summary()
    ...
    def _print_summary(self):
        print(f"\nModel Architecture (v8 Enhanced):")
        ...
        print(f"  P4d Marginal MaxEnt: {'ON' if self.use_marginal_entropy else 'OFF'}, "
              f"SCALE={self.marginal_entropy_scale}")
    ...
    def generate(self, prompt=None, length=20, verbose=False):
        """Generate text with v8 principled MCMC + Marginal MaxEnt."""
    ...
    def generate_with_trace(self, prompt=None, length=20):
        ...
        marginal_stats = {}
        if self.sampler.marginal_entropy is not None:
            marginal_stats = self.sampler.marginal_entropy.get_stats()
    ...
    def load(cls, directory):
        ...
        model.sampler = EnhancedV8Sampler(
            ...
            marginal_entropy_scale=model.marginal_entropy_scale,
            use_marginal_entropy=model.use_marginal_entropy,
        )
